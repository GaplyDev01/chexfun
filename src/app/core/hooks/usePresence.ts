...
export function usePresence(address: string | null) {
  ...
  await supabase.from('users').update({ last_seen: new Date().toISOString() }).eq('wallet_address', address);
    ...
    async function fetchOnlineUsers() {
      ...
      const now = Date.now();
      const online = (data || []).filter((u: { last_seen: string | null }) => u.last_seen && (now - new Date(u.last_seen).getTime()) < 2 * 60 * 1000).map((u: { wallet_address: string | null }) => u.wallet_address || "");
